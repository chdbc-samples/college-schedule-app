...
if "%RESOURCE_GROUP%"=="" set "RESOURCE_GROUP=library-rg"
...
az ad sp create-for-rbac `
  ...
  --scopes "/subscriptions/$subscriptionId/resourceGroups/library-rg/providers/Microsoft.ContainerService/managedClusters/library-aks" `
  --sdk-auth
...
kubectl wait --for=delete pod -l app=library-app --timeout=120s
...
kubectl get service library-app --output jsonpath='{.status.loadBalancer.ingress[0].ip}'
