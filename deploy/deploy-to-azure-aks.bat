...
if "%RESOURCE_GROUP%"=="" set "RESOURCE_GROUP=car-schedule-rg"
...
az aks create `
  --resource-group car-schedule-rg `
  --name car-schedule-aks `
  --node-count 2 `
...
az ad sp create-for-rbac `
  ...
  --scopes "/subscriptions/$subscriptionId/resourceGroups/car-schedule-rg/providers/Microsoft.ContainerService/managedClusters/car-schedule-aks" `
  --sdk-auth
...
kubectl delete deployment car-schedule-app --ignore-not-found=true

REM Чекаємо поки старі поди додатку повністю видаляться (таймаут 2 хвилини)
kubectl wait --for=delete pod -l app=car-schedule-app --timeout=120s
...
kubectl wait --for=condition=ready pod -l app=car-schedule-app --timeout=600s

REM Отримуємо зовнішню IP-адресу сервісу для доступу до додатку
kubectl get service car-schedule-app --output jsonpath='{.status.loadBalancer.ingress[0].ip}'
