...
if "%RESOURCE_GROUP%"=="" set "RESOURCE_GROUP=car-rg"
...
az ad sp create-for-rbac `
  ...
  --scopes "/subscriptions/$subscriptionId/resourceGroups/car-rg/providers/Microsoft.ContainerService/managedClusters/car-aks" `
  --sdk-auth
...
kubectl delete deployment car-app --ignore-not-found=true

REM Чекаємо поки старі поди додатку повністю видаляться (таймаут 2 хвилини)
kubectl wait --for=delete pod -l app=car-app --timeout=120s
...
kubectl wait --for=condition=ready pod -l app=car-app --timeout=600s

REM Отримуємо зовнішню IP-адресу сервісу для доступу до додатку
kubectl get service car-app --output jsonpath='{.status.loadBalancer.ingress[0].ip}'
